...
describe('Todo6: HomePage Control (17%)', () => {
    // 1st test
    it('Correct HTML relationship (7%)', () => {
      //container exists
      cy.visit('/')
    ...
    it('Error checking and prevent starting the game (5%)', () => {
        cy.get('.controlCol:nth-child(1) > input')
        ...
        cy.get('.controlCol:nth-child(1) > input')
            .then(input => changeRangeInputValue(input)(100))
        cy.get('.controlCol:nth-child(2) > input')
            .then(input => changeRangeInputValue(input)(18))
        cy.get('.controlWrapper > div:nth-child(1)').should('have.css','color','rgba(0, 0, 0, 0)')

        cy.get('.controlCol:nth-child(1) > input')
            .then(input => changeRangeInputValue(input)(48))
        cy.get('.controlCol:nth-child(2) > input')
            .then(input => changeRangeInputValue(input)(7))
        cy.get('.controlWrapper > div:nth-child(1)').should('have.css','color','rgba(0, 0, 0, 0)')

        cy.get('.controlCol:nth-child(1) > input')
            .then(input => changeRangeInputValue(input)(49))
        cy.get('.controlCol:nth-child(2) > input')
            .then(input => changeRangeInputValue(input)(7))
        cy.get('.controlWrapper > div:nth-child(1)').should('have.css','color','rgba(0, 0, 0, 0)')
